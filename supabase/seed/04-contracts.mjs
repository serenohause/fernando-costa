...
async function main() {
  ...
  const { data: foreign, error: fError } = await db
    .from('tenants')
    .select('slug, name')
    .neq('slug', TEST_TENANT_SLUG)
    .limit(5)
  if (fError) fail(`não consegui verificar os tenants: ${fError.message}`)
  if (foreign.length > 0) {
    fail(`existe tenant que não é o de teste:\n` + foreign.map((t) => `    - ${t.slug}`).join('\n'))
  }

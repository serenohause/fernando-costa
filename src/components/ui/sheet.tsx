...
function SheetOverlay({ className, ...props }: React.ComponentProps<typeof SheetPrimitive.Overlay>) {
  return (
    <SheetPrimitive.Overlay
      ...
      style={{ zIndex: 40000, pointerEvents: 'none' }}
      {...props}
    />
  )
}
...
const sheetVariants = cva(
  'fixed gap-4 bg-card p-6 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out',
  {
    variants: {
      side: {
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        className={cn(sheetVariants({ side }), className, 'max-h-[100dvh] overflow-y-auto')}
        style={{ zIndex: 40001 }}
        {...props}
      >
        {!hideClose && (

...
import { ArrowDown, ArrowUp, Check, Columns3, Pencil, X } from 'lucide-react'
...
export default function KanbanSection({ canEdit }: { canEdit: boolean }) {
  ...
  */
  const handleToggleActive = (column: KanbanColumnRow) => {
    const abertas = column.phase ? (counts[column.phase] ?? 0) : 0
    ...
    <div>
      <div className="mb-4">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          As etapas do Fluxo do Projeto: nome, cor, ordem e quanto cada uma representa do progresso.
        </p>
      </div>
      ...
      <p className="text-xs text-faint mt-3">
        Não há como criar ou excluir etapa nesta versão: as tarefas são gravadas na etapa em que
        estão, e uma etapa nova ainda não teria como recebê-las. Para tirar uma etapa do quadro sem
        perder o que está nela, esconda — o gesto é reversível.
      </p>
